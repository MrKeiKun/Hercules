...
def parse_config(input_lines, defaults):
	output = {}
	...
	for line in input_lines:
		...
		if m:
			variable, value = m.group(1), m.group(2)
			if variable in defaults:
				if defaults[variable]['parse'](variable, value, defaults[variable], output):
					continue
				sys.stdout.write("Error: Invalid value for setting '%s: %s'\n" % (variable, value))
				continue
			else:
				sys.stdout.write("Found unhandled configuration setting: '%s: %s'\n" % (variable, value))
				continue
		elif blank_or_comment_re.match(line):
			continue
		else:
			sys.stdout.write("Error: Unable to parse line '%s'\n" % line)

	return output


def cfg_add(variable, value, default, output):
	if value != default['default']:
		output[variable] = {'value': value, 'print': default['print'], 'path': default['path']}


def cfg_append(variable, value, default, output):
	for default_value in default['default']:
		if value == default_value:
			return
	if variable not in output:
		output[variable] = {'value': [], 'print': default['print'], 'path': default['path']}
...
def parsecfg_string_sub(variable, value, default, output, func):
	m = _string_quoted_re.search(value)
	if m:
		func(variable, m.group(1), default, output)
		return True
	m = _string_bare_re.search(value)
	if m:
		func(variable, m.group(1), default, output)
		return True
	return False
...
def parsecfg_string(variable, value, default, output):
	return parsecfg_string_sub(variable, value, default, output, cfg_add)


def parsecfg_stringarr(variable, value, default, output):
	return parsecfg_string_sub(variable, value, default, output, cfg_append)


# NOTE: none of these three alternatives use the /i flag in the original, so
# the hex digits must be uppercase (0x1d would NOT match) and 'no'/'false'/'off'
# must be lowercase, exactly as in the Perl source.
_int_dec_re = re.compile(r'\s*(-?[0-9]+)\s*(?://.*)?$')
_int_hex_re = re.compile(r'\s*(0x[0-9A-F]+)\s*(?://.*)?$')
_int_word_re = re.compile(r'\s*(no|false|off)\s*(?://.*)?$')


def parsecfg_int(variable, value, default, output):
	m = _int_dec_re.search(value)
	if m:
		cfg_add(variable, int(m.group(1)), default, output)
		return True
	m = _int_hex_re.search(value)
	if m:
		cfg_add(variable, int(m.group(1), 16), default, output)
		return True
	m = _int_word_re.search(value)
	if m:
...
# These two, unlike parsecfg_int's, DO use /i in the original.
_bool_true_re = re.compile(r'\s*(yes|true|1|on)\s*(?://.*)?$', re.IGNORECASE)
_bool_false_re = re.compile(r'\s*(no|false|0|off)\s*(?://.*)?$', re.IGNORECASE)


def parsecfg_bool(variable, value, default, output):
	# Note: the stored value is the *string* "true"/"false" (not a Python
	# bool), matching the original, since printcfg_bool() just interpolates it.
	m = _bool_true_re.search(value)
	if m:
		cfg_add(variable, "true", default, output)
		return True
	m = _bool_false_re.search(value)
	if m:
...
def print_config(config):
	for variable in config:
		fullpath = config[variable]['path']
		if re.search(r'[:/]$', fullpath):
			fullpath = fullpath + variable
		if ':' not in fullpath:
			continue
		filename, configpath = fullpath.split(':', 1)
		if not filename or not configpath:
			continue
		...
		# A brand new tree is built for *each* setting individually (matching
		# the original, which declares %output fresh inside this loop), so
		# settings sharing a common path prefix are NOT merged into a single
		# printed block - each one gets its own full nested example.
		output = {}
		setting = output
		while len(path) > 1:
			nodename = path.pop(0)
			if nodename not in setting:
				setting[nodename] = {'print': printcfg_tree, 'value': {}}
			setting = setting[nodename]['value']
		setting[path[0]] = {'print': config[variable]['print'], 'value': config[variable]['value']}

		verbose(
			"- Found setting: '%s'.\n  Please manually move the setting to '%s.conf' as in the following example:\n" % (variable, filename),
			"- '%s.conf': (from %s)\n" % (filename, variable)
		)
		for key in output:
			output[key]['print'](key, output[key]['value'], 0)


def indent(message, nestlevel):
	sys.stdout.write("\t" * (nestlevel + 1) + message)


def printcfg_tree(variable, value, nestlevel):
	indent("%s: {\n" % variable, nestlevel)
	for key in value:
		value[key]['print'](key, value[key]['value'], nestlevel + 1)
	indent("}\n", nestlevel)


def printcfg_nil(variable, value, nestlevel):
	pass


def printcfg_string(variable, value, nestlevel):
	indent('%s: "%s"\n' % (variable, value), nestlevel)


def printcfg_int(variable, value, nestlevel):
	indent("%s: %s\n" % (variable, value), nestlevel)


def printcfg_hexint(variable, value, nestlevel):
	indent("%s: 0x%x\n" % (variable, value), nestlevel)


def printcfg_bool(variable, value, nestlevel):
	indent("%s: %s\n" % (variable, value), nestlevel)


def printcfg_point(variable, value, nestlevel):
	indent("%s: {\n" % variable, nestlevel)

	point = (value.split(',', 2) + ['', '', ''])[:3]
	indent('map: "%s"\n' % point[0], nestlevel + 1)
	indent("x: %s\n" % point[1], nestlevel + 1)
	indent("y: %s\n" % point[2], nestlevel + 1)
...
def _perl_truthy(s):
	# Perl truthiness for a plain string: false only for "", "0", or undef.
	return s is not None and s != "" and s != "0"


def printcfg_items(variable, value, nestlevel):
	indent("%s: (\n" % variable, nestlevel)
	...
	while len(items) >= 3:
		item_id = items.pop(0)
		amount = items.pop(0)
		stackable = "true" if _perl_truthy(items.pop(0)) else "false"
		indent("{\n", nestlevel)
		indent("id: %s\n" % item_id, nestlevel + 1)
		indent("amount: %s\n" % amount, nestlevel + 1)
		indent("stackable: %s\n" % stackable, nestlevel + 1)
		indent("},\n", nestlevel)

	indent(")\n", nestlevel)


def printcfg_md5hash(variable, value, nestlevel):
	indent("%s: (\n" % variable, nestlevel)

	for entry in value:
		parts = entry.split(',', 1)
		group_id = parts[0].strip()
		md5hash = (parts[1] if len(parts) > 1 else '').strip()

		indent("{\n", nestlevel)
		indent("group_id: %s\n" % group_id, nestlevel + 1)
		indent('hash: "%s"\n' % md5hash, nestlevel + 1)
		indent("},\n", nestlevel)
...
def printcfg_strlist(variable, value, nestlevel):
	indent("%s: (\n" % variable, nestlevel)

	if isinstance(value, list):
		# NOTE (deviation, unavoidable): in the original Perl, printcfg_strlist()
		# is written for a scalar value and does split(/,/, $value). It is used
		# both for settings parsed with parsecfg_string (value is a plain string,
		# e.g. dnsbl_servers) and, for 'allow'/'deny' in packet.conf, for settings
		# parsed with parsecfg_stringarr (value is an ARRAY REF there). In the
		# latter case Perl's split() stringifies the array ref (e.g.
		# "ARRAY(0x55d1e3...)") and "splits" that address string - a latent bug
		# producing a meaningless printed value that has no Python equivalent
		# (Python has no comparable object-identity string). Here, when the value
		# is already a list, each already-parsed entry is printed directly
		# instead, which reproduces the tool's evident intent for 'allow'/'deny'.
		strings = value
	else:
		strings = value.split(',')
...
def process_conf(files, defaults):
	found = 0
	for file in files:
		sys.stdout.write("\nChecking %s..." % file)
		try:
			...
			# File not found or already converted
			sys.stdout.write(" Ok\n")
			continue
		sys.stdout.write(" Old file is still present\n")
		try:
			output = parse_config(fh, defaults)
		finally:
			fh.close()
		count = len(output)
		sys.stdout.write("%d non-default settings found." % count)
		if not count:
			verbose(" The file '%s' is no longer used by Hercules and can be deleted.\n" % file, "\n")
			continue
		verbose(" Please review and migrate the settings as described, then delete the file '%s', as it is no longer used by Hercules.\n" % file, "\n")
		print_config(output)
		found = 1
	return found


def verbose(verbose_message, silent_message=None):
	if not silent:
		sys.stdout.write(verbose_message)
	elif silent_message is not None:
		sys.stdout.write(silent_message)
...
BANNER = r"""=============== Hercules Configuration Migration Helper ===============
=               _   _                     _                           =
=              | | | |                   | |                          =
...
def main():
	...
	if confpath is None or not os.path.isdir(confpath):
		script = sys.argv[0]
		sys.stderr.write(
			"Usage: ./%s [-q | -v] [path to the conf directory]\nIf no options are passed, it acts as if called as ./%s conf\n" % (script, script)
		)
		sys.exit(1)

	count = 0
	for group in DEFAULTS:
		files = [confpath + '/' + f for f in group['files']]
		count += process_conf(files, group['settings'])
