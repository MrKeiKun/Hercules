...
static bool clif_show_monster_hp_bar(struct block_list *bl) {
	...
	if (bl->type == BL_MOB) {
		...
		if (status_get_hp(bl) < status_get_max_hp(bl)) {
			if (((battle->bc->show_monster_hp_bar & 1) && !(md->class_ == MOBID_EMPELIUM) && !(md->status.mode & MD_BOSS))
				|| ((battle->bc->show_monster_hp_bar & 2) && (md->class_ == MOBID_EMPELIUM))
				|| ((battle->bc->show_monster_hp_bar & 4) && (md->status.mode & MD_BOSS) && !(md->class_ == MOBID_EMPELIUM))
				) {
				return true;
			}
		}
